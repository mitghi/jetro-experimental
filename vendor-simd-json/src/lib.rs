// Upstream uses `#![deny(warnings)]`, but newer rustc surfaces lints upstream
// hadn't seen in 0.13.x.  Demoted to allow so the vendored fork builds across
// rustc versions without per-release patches.
#![allow(warnings)]
...
impl Buffers {
    ...
    /// stage-1 SIMD scanning.  Populated as a side effect of `to_tape_with_buffers`
    /// or any other parse path that accepts `&mut Buffers`.
    ///
    /// Added by jetro-experimental fork — upstream simd-json 0.13 keeps this
    /// field private.  Used to align our `StructuralIndex` columns with the
    /// tape walk in a single pass.
    #[cfg_attr(not(feature = "no-inline"), inline)]
    #[must_use]
